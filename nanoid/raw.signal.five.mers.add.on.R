...
### build.raw.signal.five.mers.add.on.list ###
build.raw.signal.five.mers.add.on.list = function(read.name){
  try({
    ...
    results = c(read.name,
                sapply(five.mers,function(x){apply(means.no.move[events.no.move[,"read.sequence.five.mers"] %in% x,"reevent.raw.means",drop = FALSE],2,mean)})
    )
    names(results) = NULL
    return(results)
  },silent = TRUE)
...
if(file.exists(where="data/raw.signal.five.mers.add.on.list.llo"))
{
   start = getListLength("data/raw.signal.five.mers.add.on.list.llo")/100 + 1
   print(paste0("First index: ",start))
}else{
  raw.signal.five.mers.add.on.list = list()
  saveList(object = raw.signal.five.mers.add.on.list, file = "data/raw.signal.five.mers.add.on.list.llo", append = FALSE, compress = FALSE)
  start = 1
}
...
for (j in start:length(index.list)){
  ...
  raw.signal.five.mers.add.on.tmp = foreach(n = bamNames[index.list[[j]]],.noexport = setdiff(ls(),c("bam","read.to.fast5.name","read.sequence.list","sequencing.summary"))) %dopar% build.raw.signal.five.mers.add.on.list(n)
  saveList(object = raw.signal.five.mers.add.on.tmp, file = "data/raw.signal.five.mers.add.on.list.llo", append = TRUE, compress = FALSE)
  
  rm(raw.signal.five.mers.add.on.tmp)
...
raw.signal.five.mers.add.on.list <- readList("data/raw.signal.five.mers.add.on.list.llo")
names(raw.signal.five.mers.add.on.list) <- bamNames

raw.signal.five.mers.add.on = t(sapply(raw.signal.five.mers.add.on.list,c))
colnames(raw.signal.five.mers.add.on) = col.names
rownames(raw.signal.five.mers.add.on) = raw.signal.five.mers.add.on[,"read.name"]

raw.signal.five.mers.add.on <- raw.signal.five.mers.add.on[,setdiff(col.names,"read.name")]
storage.mode(raw.signal.five.mers.add.on) <- "numeric"

save(raw.signal.five.mers.add.on,file=file.path("data","raw.signal.five.mers.add.on.RData"))

saveList(object = raw.signal.five.mers.add.on.list, file = "data/raw.signal.five.mers.add.on.list.llo", append = FALSE, compress = FALSE)
# system("rm data/raw.signal.five.mers.add.on.list.llo")
